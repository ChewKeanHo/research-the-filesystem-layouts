...
* Provisioning data sanitization functions for `Controllers` to call.
* Provisioning data processing functions for `Controllers` to call.
...
* **Handles business logics** - that is `Controllers` layer's role.
* **Handles response rendering logics** - that is `Views` layer's role.
* **Called by `Views`** - avoid that. That is `Controllers` layer's role.
