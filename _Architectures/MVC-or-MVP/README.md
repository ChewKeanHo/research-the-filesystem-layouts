...
`Controllers` and `Presenters`. Such difference is mainly about grammar and
political jargons where one argues:

1. `Controllers` hands over the control flow to the `Models` and the `Models`
   deal with the response completely; AND
2. `Presenters` handles the control flow entirely and both `Models` and `Views`
   are libraries to it.
...
* **Models** - holds the data models with `create`, `read`, `update`, and
  `delete` interfaces for `Controllers`|`Presenters` to call with.
* **Views**  - holds the output interface design rendering with `read` and
  `update` interfaces for `Controller`|`Presenters` to call.
* **Controllers|Presenters** - holds the coordination interfaces for the client
...
3. `Controller`|`Presenter` function parse the inputs and call the correct
   `Models` functions and process the data.
...
vulnerability especially in composited or compounded data `Models`. Due to the
simplicity of its architecture patterns, 3rd-party libraries are often directly
integrated into `Controllers`|`Presenters`.
