...
This base directory houses all operating system (OS)'s `sendmail` control files.
This means it can operate in `Single-User` mode in BSD realm or `Emergency Mode`
in Linux realm.
...
In FreeBSD, known files are:

```
/etc/mail/
  aliases       # addresses for delivery
  mailer.conf   # mailwrapper configuration
```
...
Refer `sendmail` and `mailwrapper(8)` manuals for specifications.
