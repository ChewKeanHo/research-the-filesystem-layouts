...
This base directory houses all operating system (OS)'s cron daily security check
scheduler's configuration files using the `.d` configuration directory pattern
where 1 file is 1 cron file. It can operate minimally without any mounting
...
Note that the configuration in this `security` directory is similar to:
...
`/usr/local/etc/periodic/security` instead. Therefore, generally, you
