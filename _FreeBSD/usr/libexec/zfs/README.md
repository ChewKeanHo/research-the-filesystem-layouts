...
distributor supplied, non-critical, non-commandable OpenZFS programs and
...
will break the OS updates or upgardes in any way. Use `/usr/local/libexec/zfs`
or `${HOME}/[USERNAME]/.local/libexec/zfs` instead. In the case of `/libexec`
...
Refer `zfs(8)` manual for specifications.
