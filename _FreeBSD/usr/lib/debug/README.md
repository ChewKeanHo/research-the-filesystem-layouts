...
distributor supplied, non-critical standalone kernel and base OS debugging
library files (e.g compiled linkable object files) of an OS to function
...
will break the OS updates or upgardes in any way. Use `/usr/local/lib/debug` or
`${HOME}/[USERNAME]/.local/lib/debug` instead. In the case of `/lib` being
