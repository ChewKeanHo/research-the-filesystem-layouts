...
distributor supplied, non-critical dtrace library files (e.g compiled linkable
object files) of an OS to function properly. This means it can operate in
...
will break the OS updates or upgardes in any way. Use `/usr/local/lib/dtrace` or
`${HOME}/[USERNAME]/.local/lib/dtrace` instead. In the case of `/lib` being
...
Refer `dtrace` manual for specifications.
