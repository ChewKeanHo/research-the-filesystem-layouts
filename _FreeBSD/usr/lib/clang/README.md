...
distributor supplied, non-critical clang shared library files (e.g compiled
linkable object files) of an OS to function properly. This means it can operate
...
will break the OS updates or upgardes in any way. Use `/usr/local/lib/clang` or
`${HOME}/[USERNAME]/.local/lib/clang` instead. In the case of `/lib` being
...
Refer `clang(1)` manual for specifications.
