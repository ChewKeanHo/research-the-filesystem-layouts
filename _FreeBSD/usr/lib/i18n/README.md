...
distributor supplied, non-critical internationalization (i18n) library files
(e.g compiled linkable object files) of an OS to function properly. This means
...
will break the OS updates or upgardes in any way. Use `/usr/local/lib/i18n` or
`${HOME}/[USERNAME]/.local/lib/i18n` instead. In the case of `/lib` being
