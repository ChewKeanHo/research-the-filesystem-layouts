...
distributor supplied, non-critical, CPU architecture specific library files
(e.g cross-platform compiled linkable object files) of an OS to function
...
between `/` and `/usr` layers, `/usr/lib[ARCH]` directory is being symbolic
linked to `/lib[ARCH]` instead; unifying both directories. This reduces the
separation complexities for package managements and distributions as all
packages only needs to target `/usr/lib[ARCH]` directory.

In FreeBSD, this directory is still maintaining its verbatim separated roles and
responsibilites from `/usr/lib[ARCH]` directory.
...
will break the OS updates or upgardes in any way. Use `/usr/local/lib[ARCH]` or
`${HOME}/[USERNAME]/.local/lib[ARCH]` instead. In the case of `/lib[ARCH]` being
symbolically linked from `/usr/lib[ARCH]`, you **MUST** place the content here
instead of `/lib[ARCH]`.
