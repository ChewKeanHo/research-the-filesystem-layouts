...
distributor supplied, non-critical, `pkg` used fingerprinting keys files of an
...
will break the OS updates or upgardes in any way. Use `/usr/local/share/keys/pkg`
or `${HOME}/[USERNAME]/.local/share/keys/pkg` instead.
...
Refer `pkg(7)` and `pkg(8)` manuals for specifications.
...
```
/usr/share/keys/pkg/
  trademark-product-signature.pem
  ...

OR

/usr/share/keys/pkg/
  product-signature.pem
  ...
```
