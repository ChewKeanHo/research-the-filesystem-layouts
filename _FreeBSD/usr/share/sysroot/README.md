...
distributor supplied, non-critical, non-native compiler and linker build
resource files of an OS to function properly. This means it can operate in
...
`/usr/local/share/sysroot` or `${HOME}/[USERNAME]/.local/share/sysroot`
instead.
...
This is a controlled directory structure as such:

```
/usr/share/sysroot/
  [VERSION]/                        # match `uname(1) -r`
    ...
    [MACHINE].[MACHINE_ARCH]/       # match `uname(1) -m` and `uname(1) -p`
      ...
```
