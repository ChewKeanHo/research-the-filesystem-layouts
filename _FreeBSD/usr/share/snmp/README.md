...
distributor supplied, non-critical, SNMP daemon usable resource files of an OS
to function properly. This means it can operate in `Multi-User` mode in BSD
...
will break the OS updates or upgardes in any way. Use `/usr/local/share/snmp` or
`${HOME}/[USERNAME]/.local/share/snmp` instead.
...
Refer `gensnmptree(4)` manual for specifications and file directory structures.
